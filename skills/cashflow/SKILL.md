...
| 「我能動多少錢」「到月底剩多少」「還有多少錢」「這個月安全額度是多少」 | GET /forecast/available                                  | safe available amount first, then breakdown                                                |
...
| 「列出 XX 實領紀錄」「XX 月薪 adjustments」「之前確認過幾次薪水」      | GET /incomes/{id}/adjustments                            | adjustment list (effective_date, actual_amount, delta vs base, note)                       |
...
| 「這次實領 XX」「上個月薪水其實是 XX」「確認 XX 月薪 XX」 | POST /incomes/{id}/actuals            | income name, effective_date, actual_amount, delta vs base. Pre-check: frequency != once；effective_date 對齊排班日（不對齊回 MISALIGNED_DATE） |
| 「移除 XX 月薪 actual」「把 X 月薪改回基準」            | DELETE /income-adjustments/{id}       | income name + effective_date (先 GET /incomes/{id}/adjustments 找 id)                                                                        |
...
| 「這個月薪水 XX」「實領 XX」           | Clarify: 一次性實領紀錄 (POST /incomes/{id}/actuals) 還是調基準薪水 (PUT /incomes/{id})? 預設前者；若使用者說「以後都這樣」「調薪了」則改後者 |
...
17. **帳單費用分類規則：** 信用卡帳單上除了「分期本金」（由 installment_amount 管理）
    ...
    歸類後必須告知使用者，不可靜默歸入。
18. **安全額度呈現規則：** 當使用者在意焦慮、怕把必要支出花掉、或明確詢問「安全額度」時，
    回覆必須先給單一答案，再拆成「帳面總餘額 / 不能動的預留 / 真正可自由動用」。
    若使用者有安全邊界或消費慣例預留，必須另外列出，不可混在總數裡。
19. **Income adjustment（每次發薪日實領金額）：** 不動 `Income.amount` 基準，而是針對特定 `effective_date`
    寫入 `actual_amount`。Forecast / timeline / dashboard / planning 自動以 actual 取代基準計算。
    限制：`frequency=once` 不支援（回 `INVALID_FREQUENCY`）；`effective_date` 必須對齊 Income 排班日
    （從 `start_date` 依 `frequency` 推進可達；不對齊回 `MISALIGNED_DATE`）。`delta_amount = actual − base`
    於 read 時動態計算，事後修改 `Income.amount` 仍正確。同一 `(income_id, effective_date)` 唯一，
    再次 POST 為 upsert（覆蓋既有 actual_amount + note）。
    **判斷準則：** 使用者說「這次薪水 XX」「實領 XX」→ 一次性 actual；說「調薪了」「以後都 XX」→ 改基準 (PUT /incomes)。
